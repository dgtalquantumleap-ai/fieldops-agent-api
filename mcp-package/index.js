...
// FieldOps Agent — MCP Server (stdio transport)
//
// WhatsApp-native booking, payment collection, invoice recovery,
// and staff coordination for cleaning and field service businesses.
//
// Usage:
//   npx -y @ebenova/fieldops-mcp
//
// Required env: DATABASE_URL (PostgreSQL)
// Optional env: TWILIO_* or WHATSAPP_* (for WhatsApp messaging)
...
const { generatePaymentLink, detectRegion } = require('./lib/payments');

const server = new McpServer({
  name: '@ebenova/fieldops-mcp',
  version: '1.0.0',
});
...
server.tool(
  'check_availability',
  'Check available booking slots for a given date. Returns booked and available time slots.',
  {
  ...
  async ({ date, business_id }) => {
    ...
    try {
      ...
      return {
        content: [{ type: 'text', text: JSON.stringify({ date, available_slots: available, booked_slots: booked }) }]
      };
    } catch (err) {
...
server.tool(
  'book_service',
  'Book a cleaning or service appointment. Creates the job, assigns staff, sends WhatsApp confirmation with payment link.',
  {
  ...
  async ({ customer_name, customer_phone, service, date, time, address, business_id }) => {
    ...
    try {
      ...
      await sendWhatsAppMessage(customer_phone, msg).catch(err => {
        console.error('[fieldops-mcp] WhatsApp send failed:', err.message);
      });
      ...
      return {
        content: [{
          type: 'text',
          text: JSON.stringify({
            success: true, booking_id: job.id, service: serviceRecord.name,
            date, time, address, staff: staff?.name || 'TBD',
            amount: serviceRecord.price, payment_link: paymentLink
          })
        }]
      };
    } catch (err) {
...
server.tool(
  'send_payment_link',
  'Generate and send a payment link to a customer via WhatsApp. Supports CAD (Polar/Stripe) and NGN (OxaPay).',
  {
    customer_phone: z.string().describe('Customer WhatsApp number in E.164 format'),
  ...
  async ({ customer_phone, job_id, amount, currency }) => {
    try {
      ...
      const link   = await generatePaymentLink({
        jobId: job_id, amount, customerName: 'Customer',
        customerPhone: customer_phone, region, description: `Job #${job_id}`
      });
      ...
      return { content: [{ type: 'text', text: JSON.stringify({ success: true, payment_link: link }) }] };
    } catch (err) {
      return { content: [{ type: 'text', text: JSON.stringify({ error: err.message }) }] };
    }
  }
...
server.tool(
  'recover_invoice',
  'Send an escalating WhatsApp payment recovery message for an unpaid invoice. Step 1=polite reminder, 2=firm follow-up, 3=final notice.',
  {
  ...
  async ({ invoice_id, step }) => {
    try {
      ...
      const link = await generatePaymentLink({
        jobId: inv.job_id, amount: inv.amount,
        customerName: inv.customer_name, customerPhone: inv.customer_phone,
        region: detectRegion(inv.customer_phone), description: `Invoice ${inv.invoice_number}`
      }).catch(() => null);
      ...
      return {
        content: [{
          type: 'text',
          text: JSON.stringify({
            success: true, invoice_id, customer: inv.customer_name,
            amount: inv.amount, step, message_sent: true, payment_link: link
          })
        }]
      };
    } catch (err) {
...
server.tool(
  'send_staff_briefing',
  'Send a WhatsApp job briefing to the assigned staff member with service details, customer info, and location.',
  {
...
async function main() {
  ...
  console.error('[mcp] @ebenova/fieldops-mcp running on stdio');
}

main().catch(err => {
  console.error('[fieldops-mcp] Server error:', err);
  process.exit(1);
});
