// lib/payments.js — Payment link generation (OxaPay for NG, Polar for CA/US)
...
async function generatePaymentLink({ jobId, amount, customerName, customerPhone, region, description }) {
  ...
  try {
    ...
    console.error('[fieldops-mcp] Payment link error:', err.message);
    return `${process.env.APP_URL || 'https://fieldops.ebenova.dev'}/pay?job=${jobId}&amount=${amount}`;
  }
}

async function createOxaPayLink({ jobId, amount, description }) {
  const appUrl = process.env.APP_URL || 'https://fieldops.ebenova.dev';
  const res = await fetch('https://api.oxapay.com/merchants/request', {
    ...
    body: JSON.stringify({
      ...
      lifeTime:    60,
      feePaidByPayer: 1,
      callbackUrl: `${appUrl}/webhook/payments/oxapay`,
      returnUrl:   `${appUrl}/booking-status.html?job=${jobId}`,
...
async function createPolarLink({ jobId, amount, customerName, customerPhone, description }) {
  ...
  const res = await fetch('https://api.polar.sh/v1/checkouts/', {
    ...
    body: JSON.stringify({
      ...
      currency:    'cad',
      success_url: `${appUrl}/booking-status.html?job=${jobId}`,
